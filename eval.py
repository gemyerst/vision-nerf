...
class SRNRenderDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        src_rgbs = []
        src_masks = []
        for rgb_path in src_rgb_paths:  #adapted code to only need 1 image to take in
...
def make_transparent_png(img_in): #own code
    # resize image
    ...
    for d_idx in data_index:
        ...
        with torch.no_grad():
            ...
            for idx, pose in tqdm.tqdm(zip(view_indices, render_poses), total=len(view_indices)):
                ...
                cleaned_rbg = make_transparent_png(rgb_im) #own code
                imageio.imwrite(filename, cleaned_rbg)
                imgs.append(rgb_im) #own code
                torch.cuda.empty_cache()

            imgs = np.stack(imgs, 0)
            imageio.mimsave(os.path.join(out_folder, f'output.gif'), imgs, fps=12) #own code
